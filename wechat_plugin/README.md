...
| `分析以色列` | 当日深度分析（纯文本） |
| `加沙本周分析` / `本周加沙` | 7 天叙事分析 |
| `分析以色列 图片` | Playwright 截图 HTML 报告（PNG）|
| `分析以色列 PDF` / `报告` | A4 PDF 完整版（Playwright 打印） |
| `今日热点` / `热度榜` | 全球新闻热度榜 Top 15 |
| `中国新闻` | 单国文章标题列表 |
| `历史简报` | briefs 列表 |
| `帮助` / `help` | 速查 |
| `测试推送` | 立即触发"每日推送"流程（用于验证 scheduler） |
| `测试告警` | 立即触发"热点告警"检查 |
| 其他文本 | CoW 默认 LLM 闲聊 |

## 定时推送说明

- **每日推送**：默认 8:00（可在 plugin config.json 改 `daily_push_cron_hour/minute`）
- **热点告警**：每小时检查一次，某国当日 ≥5 篇且为昨日 3 倍以上才告警
- **主动推送的前提**：目标用户**至少给 bot 发过一条消息**，bot 才能拿到该用户的会话上下文。
  自用场景下，自己先发一条任意消息（如 `帮助`），之后定时推送就能到自己。
